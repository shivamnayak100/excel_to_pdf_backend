...
const fs = require('fs');
const XLSX = require('xlsx');  // For handling .xls and .xlsx files
...
app.post('/convert', upload.array('file', 10), async (req, res) => {
  try {
      ...
      if (fileExtension === '.xlsx') {
        ...
        pdfStream.on('finish', () => {
          res.download(pdfPath, (err) => {
            // After download, remove both the uploaded Excel file and generated PDF
            removeFile(filePath);
            removeFile(pdfPath);
          });
        });
